...
def display_results():
    ...
    output_icons = len([f for f in os.listdir('build/output') if f.startswith('icon_')])
    extracted_shapes = len([f for f in os.listdir('build/extracted_shapes') if f.endswith('_shape.png')])
    
    if os.path.exists('new_icons'):
        final_icons = len([f for f in os.listdir('new_icons') if f.startswith('merged_icon_')])
    else:
        final_icons = 0
...
def main():
    ...
    # Step 3: Final assembly (background removal)
    success = run_step(
        "3/3",
        "final_assembly",
        "Removing backgrounds and creating final icons"
    )
